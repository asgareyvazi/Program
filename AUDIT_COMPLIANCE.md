...
| Testing | 3.5 | ✅ **۱۳۵۶+ تست در ۱۱ سویت + ۲۴۰ آیتم Audit + ۲۰ آیتم Data Audit** | `tests/run_all.py` (255 مرجع + 74 حاکمیتی + 51 قالب + 30 UI + 54 API + 72 کیفیت + 721 خروجی + 61 یکپارچگی + 18 Registry + **20 Data Usage**) |
...
`(این کامیت — Batch AK)` (**ترکیب ریزدانهٔ سند + ممیزی استفاده از دادهها**: `wizard_compose.py` — کاربر خیلی جزئی انتخاب میکند: **حفرهها** (سایز/عمق/کیسینگ + پروسیجر حفاری/لولهگذاری/سیمان/چکلیست برای هر حفره از دیتابیس دستهبندیشده)، **پروسیجرهای انتخابی** از ۱۶ دسته، **تکمیل** (نوع + تولزها + پروسیجر تکمیل)، و **اسناد دانش انتخابی** از کاتالوگ ۷۵۴تایی؛ دیالوگ ۴تبی «⚙️ Fine-Grained Composition» در صفحهٔ Sections و افزودن خودکار به سند؛ `tests/data_usage_audit.py` — ممیزی کامل بودن دستهبندی (۱۶ دستهٔ پروسیجر بدون دستهٔ خالی، ۵ بعد کاتالوگ، CBS، مشکلات) و قابلدسترسبودن همهٔ دادهها از UI (۱۸۶+ پروسیجر و ۷۵۴ سند)؛ ۱۲ تست یکپارچگی + ۱ تست UI + ۲۰ آیتم Data Audit)
`(این کامیت — Batch AH+AI+AJ)` (**تکدیتابیس + نصبکننده + T&D stiff-string**: `unified_db.py` — ادغام همهٔ ۹ دیتابیس در `unified.db` با جدولهای prefixدار و جدول `entity_links` با FK واقعی + `integrity_report()` که ارجاعهای بینپایگاهی (procedure→well، daily→well، NPT→well...) را تأیید میکند و ارجاع شکسته را پیدا میکند (با تست orphan) + سکشن «UNIFIED DATABASE REPORT» + `/api/unified`؛ `installer.py` — نصب/چک/تعمیر بدون PyInstaller (deps + bootstrap + لانچرهای ویندوز/لینوکس + دسکتاپ entry) — `python installer.py --check`؛ `engineering_td.py` — مدل **stiff-string سهبعدی** (سهنقطهتماس + خمش/DLS + گشتاور خمشی) که همیشه ≥ soft-string است و در قائم صفر میشود — ۶ تست مرجع + ۱ تست حاکمیتی + ۱ تست API)
`(این کامیت — Batch AG)` (**Daily Drilling Report — First-Class Plan vs Actual**: `daily_report.py` — سند «DAILY DRILLING REPORT» به سبک Morning Report (خلاصهٔ عملیات، عمق/واریانس، پارامترهای حفاری، NPT با علت) + سکشن «PLAN vs ACTUAL — DAILY VARIANCE» با جدول واریانس روزانه و پرچمهای KPI (⚠️ عمق، 🐢 ROP، 🕘 NPT) از سوابق operations.db؛ منوی «Tools → Daily Drilling Report»؛ اندپوینت `/api/daily-report`؛ ۷ تست مرجع + ۱ تست API)
`(این کامیت — Batch AF)` (**Cross-Well Intelligence — یادگیری بینچاهی**: `well_intelligence.py` — رتبهبندی چاههای ذخیرهشده بر اساس شباهت مهندسی (عمق/سایز/MW/نوع/میدان)، **پیشنهاد ورودی از مشابهترین چاه آفست**، سکشن «OFFSET WELL COMPARISON»، گزارش «WELL DATABASE REPORT»، و سکشن «Offset-Well Intelligence» در Deep Verification هر سند؛ در ویزارد: دکمههای «📂 Load Inputs from Stored Well» و «✨ Suggest from Similar Offset Well» در صفحهٔ ورودیها؛ اندپوینتهای `/api/wells/similar` و `/api/wells/report`؛ ۷ تست مرجع + ۲ تست API + ۱ تست UI)
`(این کامیت — Batch AE)` (**Well Model بهعنوان منبع واقعی — ذخیره/بازیابی در تولید سند**: هر تولید سند، Basis چاه را در `wells.db` ذخیره میکند؛ `save_well` با **upsert بر اساس نام چاه** (بدون ردیف تکراری) و نگهداری **تاریخچهٔ ۲۵ revision**؛ `load_well_values()` چاه ذخیرهشده را به dict ورودی ویزارد برمیگرداند (برای پیشپر کردن یا مقایسهٔ بینچاهی)؛ تست یکپارچگی ۱۵)
`(این کامیت — Batch AD)` (**Canonical Input Registry — منبع واحد کلید/واحد/محدوده**: `input_registry.py` با ۳۸ کمیت کانونی، ~۹۰ alias، واحد و محدودهٔ مهندسی؛ Validation/Standards/Consistency/Register/Deep همگی مقادیر را از طریق registry میخوانند (fracture_gradient، formation_pressure، flow_rate_gpm، q_gpm، sidpip، mud_yp و...)؛ عمق در همهٔ موتورها بهصورت کانونی ft (تبدیل دوباره غیرممکن)؛ سویت `tests/registry_audit.py` (۱۸ تست) که یکسانی دید همهٔ موتورها و نبود تبدیل دوباره را قفل میکند)
`(این کامیت — Batch AC)` (**رفع باگهای UI تأییدشدهٔ ممیزی + Import WITSML در ویزارد**: کاراکتر «&» در عنوان تبها/هدرها دیگر توسط mnemonic کیوت بلعیده نمیشود (Company & Well، Formations & Hazards، BOP & Well Control، DRILLING PROGRAM & PROCEDURE GENERATOR...)؛ هدر دیگر در عرضهای کوچک بریده نمیشود (word wrap + حداقل عرض 1150px برای لپتاپهای 1366px)؛ منوی «File → Import WITSML» فایل WITSML را میخواند و **ویزارد را با نام چاه/میدان/عمق آب/جدول مسیر از پیشپر میکند** (run_wizard(initial_values=...) + prefill در InputsPage)؛ ۲ تست UI + ۱ تست یکپارچگی)
`(این کامیت — Batch AB)` (**Torque & Drag با کالیبراسیون + تمایز نوع کیک + choke-line فراساحلی**: `engineering_td.py` — مدل soft-string (بار قلاب در/خروج، تورک سطحی، حد کمانش حلزونی Dawson-Paslay) + **کالیبراسیون friction factor از دادهٔ چاههای مجاور** (حداقل مربعات، بازیابی μ=0.40 در تست) — همان الگوی ROP؛ **طبقهبندی سیال کیک (گاز/آب/روغن)** از اختلاف SICP−SIDPP با آستانههای گرادیان + گام «Kick fluid classification» در سناریو + ورودی kick_type؛ **friction خط چوک دیپواتر/MPD** (هر دو رژیم laminar/turbulent)؛ ۱۵ تست جدید)
`(این کامیت — Batch AA)` (**سازگاری بینبخشی + محدودههای مهندسی + سکشن presence دقیق + provenance**: `engineering_consistency.py` — اسکن نهایی سند برای تناقضها (casing>TD، MW>FG، ECD>FG، BOP<MASP، مقادیر تکراری متضاد) با سکشن «CROSS-DOCUMENT CONSISTENCY» + `/api/consistency`؛ **محدودههای واقعبینانهٔ مهندسی** در Schema Validation (MW 6-22، BOP 2000-20000، دبی، RPM، WOB، TFA، سایزها...) — ورودی ۵۰۰۰۰ ppg دیگر قابل قبول نیست؛ **سکشن presence مبتنی بر heading** (جستجوی عبارت ساده دیگر سکشن حساب نمیشود — رفع false positive ممیزی)؛ ستون **Origin** در Calculation Register (CALCULATED/USER-ENTERED/ESTIMATED)؛ ۱۰ تست جدید)
`(این کامیت — Batch Z)` (**WITSML Import + IADC Dull Grading + بستهبندی سازمانی**: `witsml_import.py` — دریافت WITSML (چاه/میدان/مسیر) و ادغام در ورودیها؛ `iadc_dull.py` — تفسیر کد ۸کاراکتری IADC Dull Grading (هر دو فرم dash و فشرده) با ارزیابی وضعیت (ادامه/تعویض) و سکشن سند؛ اندپوینتهای `/api/witsml/import` و `/api/iadc-dull`؛ `packaging/DrillingProgram.spec` + اسکریپتهای build ویندوز/لینوکس برای توزیع سازمانی (PyInstaller با کتابخانهٔ دانش bundle)؛ ۱۹ تست جدید)
