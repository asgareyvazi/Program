...
- **🧪 تست خودکار (۱۶۵ تست در ۴ سویت)** — اجرای همه: `python tests/run_all.py`؛ ۶۹ تست مرجع مهندسی با تلورانس پذیرش (Hydrostatic، MAASP، KMW، Annular Velocity، Casing Burst API، Well Cost و...) + ۲۸ تست حاکمیتی (بکاپ/رمزنگاری/Revision Snapshots/Register) + رگرسیون ۵۱/۵۱ قالب end-to-end (تولید Word واقعی + ۵ سکشن حاکمیتی + اسکن نشت نام) + ۱۷ تست UI آفلاین
